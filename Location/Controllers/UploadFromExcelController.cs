...
namespace Location.Controllers
{
    public class UploadFromExcelController : Controller
    {
        ...
        public FileResult ExportToExcel()
        {
            ...
            dt.Columns.AddRange(new DataColumn[4] { new DataColumn("Nom"),
            ...
            var demandes = from Demande in db.Demandes select Demande;

            foreach (var dem in demandes)
            {
                dt.Rows.Add(dem.Nom , dem.Prenom, dem.Telephone, dem.Courriel, dem.HeureRdv, "", "");
            }
        ...
        public ActionResult ImportFromExcel(HttpPostedFileBase postedFile)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("postedFile", "votre fichier depasse la limite acceptable qui est de 50MB !");
                }
                    ...
                    try
                    {
                        ...
                        using (SqlConnection con = new SqlConnection(conString))
                        {
                            using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(con))
                            {
                                ...
                                sqlBulkCopy.DestinationTableName = "Demandes";
                                con.Open();
